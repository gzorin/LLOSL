...
CreateLoadPacked(llvm::IRBuilder<T, Inserter>& builder, llvm::Value *address) {
    ...
    auto vector_type = llvm::VectorType::get(array_type->getElementType(), array_type->getNumElements());
    ...
    for (unsigned i = 0, n = array_type->getNumElements(); i < n; ++i) {
        result = builder.CreateInsertElement(
            result,
            builder.CreateLoad(builder.CreateConstGEP2_32(nullptr, address, 0, i)),
            i);
    }
...
CreateStorePacked(llvm::IRBuilder<T, Inserter>& builder, llvm::Value *value, llvm::Value *address) {
    ...
    for (unsigned i = 0, n = array_type->getNumElements(); i < n; ++i) {
        builder.CreateStore(
            builder.CreateExtractElement(value, i),
