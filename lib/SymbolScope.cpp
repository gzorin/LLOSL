...
SymbolScope::SymbolScope(LLOSLContextImpl &context, TypeScope &type_scope, Library &library,
                         llvm::IRBuilder<> &builder, llvm::Function *function,
                         llvm::Value *shader_globals)
    : d_context(context)
    ...
    if (d_function) {
        std::for_each(d_function->arg_begin(), d_function->arg_end(), [this](auto &arg) -> void {
    ...
    case SymTypeTemp: {
        ...
        if (t.is_closure_array()) {
            ...
            for (unsigned i = 0, n = t.arraylength(); i < n; ++i) {
                d_builder.CreateStore(d_context.getLLVMClosureDefaultConstant(),
        ...
        if (t.is_array()) {
            for (unsigned i = 0, n = t.arraylength(); i < n; ++i) {
                d_builder.CreateStore(d_builder.CreateExtractValue(value, std::vector<unsigned>{i}),
                                      d_builder.CreateConstGEP1_32(nullptr, address, i));
            }
...
SymbolScope::getValueForArgument(const Symbol *s) {
    return d_type_scope.isPassedByReference(s->typespec()) ? getReference(s)
...
SymbolScope::allocate(const Symbol *s) {
    ...
    llvm::Type *type = t.is_array() ? d_type_scope.get(t.elementtype()) : d_type_scope.get(t);

    llvm::Value *count = t.is_array() ? llvm::ConstantInt::get(d_context.getLLContext(),
                                                               llvm::APInt(32, t.arraylength()))
                                      : nullptr;
