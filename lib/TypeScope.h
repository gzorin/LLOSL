...
public:
    ...
    bool isPassedByReference(const OSL::TypeDesc& t) const { return d_context.isTypePassedByReference(t); }
    bool isPassedByReference(const OSL::pvt::TypeSpec&) const;

    llvm::Type *getForArgument(const OSL::TypeDesc& t) { return d_context.getLLVMTypeForArgument(t, false); }
    llvm::Type *getForArgument(const OSL::pvt::TypeSpec&);

    llvm::Constant *getDefaultConstant(const OSL::pvt::TypeSpec&);
    std::pair<llvm::Constant *, const void *> getConstant(const OSL::pvt::TypeSpec&, const void * = nullptr);
