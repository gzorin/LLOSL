...
: d_context(&context) {
    ...
    std::for_each(
        ...
        [this, &ll_context](auto tmp) -> void {
            ...
            d_bxdf_component_mapping[function] = llvm::Function::Create(
                ...
                llvm::GlobalValue::ExternalLinkage, function->getName(), d_module.get());
        });
    ...
    auto bxdf_type = llvm::FunctionType::get(
        ...
        std::vector<llvm::Type *>{
            float3_type, float3_type, llvm::Type::getInt16Ty(ll_context), void_pointer_type },
        false);

    d_function = llvm::Function::Create(
        bxdf_type, llvm::GlobalValue::ExternalLinkage, "llosl_uber_bxdf", d_module.get());
    ...
    auto default_result = llvm::ConstantVector::getSplat(
        3,
        llvm::ConstantFP::get(llvm::Type::getFloatTy(ll_context), 0.0));
...
UberBXDF::insertBXDF(const BXDF *bxdf) {
    ...
    if (it == d_index.end()) {
        ...
        auto function = llvm::Function::Create(
            bxdf->function()->getFunctionType(),
            llvm::GlobalValue::ExternalLinkage, bxdf->function()->getName(), d_module.get());
        llvm::ValueToValueMapTy mapping;

        std::for_each(
            d_bxdf_component_mapping.begin(), d_bxdf_component_mapping.end(),
        ...
        d_switch_instruction->addCase(
            llvm::ConstantInt::get(
                llvm::Type::getInt16Ty(ll_context), index, false),
            block);
