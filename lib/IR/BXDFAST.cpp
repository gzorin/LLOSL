...
BXDFAST::print(llvm::raw_ostream &s, BXDFAST::NodeRef node) {
    ...
    struct Print {
        ...
        void operator()(NodeRef, const Void &) { s << "?"; }
        ...
        void operator()(NodeRef, const Add &node) { s << "(+"; }

        void operator()(NodeRef, const MulColor &node) { s << "("; }

        void operator()(NodeRef, const MulFloat &node) { s << "("; }
    };
    ...
    struct PrintBack {
        ...
        void operator()(NodeRef, const MulColor &node) { s << "*c[" << node.rhs_address << "])"; }

        void operator()(NodeRef, const MulFloat &node) { s << "*f[" << node.rhs_address << "])"; }
    };
...
uint8_t
encode(Opcode opcode, unsigned operand) {
    return (uint8_t)opcode | ((uint8_t)operand & 0x1F);
}

std::pair<Opcode, unsigned>
decode(uint8_t code) {
...
BXDFAST::decode(BXDFAST::Encoding encoding) {
    ...
    std::function<NodeRef(Encoding::const_iterator)> detail =
        ...
        case Opcode::Component: {
            return std::make_shared<Node>(Component(operand, *it++));
        } break;
        case Opcode::WeightedComponent: {
            return std::make_shared<Node>(WeightedComponent(operand, *it++, 0));
        } break;
        case Opcode::Add: {
            return std::make_shared<Node>(Add(detail(it), detail(it_begin + operand)));
        } break;
        case Opcode::MulColor: {
            return std::make_shared<Node>(MulColor(detail(it), operand << 2));
        } break;
        case Opcode::MulFloat: {
            return std::make_shared<Node>(MulFloat(detail(it), operand << 2));
        } break;
        }
    };
