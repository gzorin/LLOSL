...
BXDFInfo::Create(const ClosureFunction &function, const PathInfo &path_info) {
    ...
    struct Frame {
        ...
        unsigned allocate(unsigned size) { return std::exchange(heap_size, heap_size + size); }
    ...
    while (!stack.empty()) {
        ...
        if (llvm::isa<ClosureBlock>(frame.block)) {
            auto block = llvm::cast<ClosureBlock>(frame.block);
            ...
            std::for_each(
                ...
                [&bxdf_info, &function, &frame](const auto &inst) -> void {
                    ...
                    case Value::ValueKind::Store: {
                        ...
                        frame.storage[store_instruction->getLocation()] =
                            frame.values[store_instruction->getOperand()];
                    } break;
                    case Value::ValueKind::AllocateComponent: {
                        ...
                        frame.values[&inst] = std::make_shared<BXDFAST::Node>(
                            BXDFAST::Component(component_instruction->getClosureID(), address));
                    } break;
                    case Value::ValueKind::AllocateWeightedComponent: {
                        ...
                        auto component_address =
                            frame.allocate(component_instruction->getClosureSize());
                        frame.values[&inst] =
                            std::make_shared<BXDFAST::Node>(BXDFAST::WeightedComponent(
                                component_instruction->getClosureID(), component_address, 0));
                    } break;
                    case Value::ValueKind::AddClosureClosure: {
                        auto add_instruction = llvm::cast<AddClosureClosure>(&inst);
                        frame.values[&inst]  = std::make_shared<BXDFAST::Node>(
                            BXDFAST::Add(frame.storage[add_instruction->getLHS()],
                                         frame.storage[add_instruction->getRHS()]));
                    } break;
                    ...
                    case Value::ValueKind::Return: {
                        ...
                        bxdf_info->addBXDFForPath(frame.path_id,
                                                  frame.storage[function.getCiLocation()],
                                                  frame.heap_size);
                    } break;
                    default:
                        break;
        ...
        std::for_each(frame.block->succs_begin(), frame.block->succs_end(),
                      [path_info, &stack, &frame](auto tmp) -> void {
                          auto [succ, edge] = tmp;
