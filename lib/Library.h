...
public:
    ...
    public:
        ...
        template <typename... Args> llvm::Value *operator()(llvm::StringRef, Args &&...);

        template <typename... Args> llvm::Value *operator()(Function &, Args &&...);

        class Callable {
        public:
            template <typename... Args> llvm::Value *operator()(Args &&...);
    ...
    private:
        ...
        template <typename... Args> llvm::Value *callv(Function &, Args &&...);
...
template <typename Collector, typename... Args> void CollectArguments(Collector &, Args &&...);
...
Library::CallingContext::callv(Function &function, Args &&... args) {
    struct Collector {
        ...
        llvm::Value *finalize() { return d_caller.call(d_function, d_args); }

        void operator()(const Symbol *s) { add(GetValueForArgument(d_symbol_scope, s)); }

        void operator()(int arg) {
            add(llvm::ConstantInt::get(d_llcontext, llvm::APInt(32, arg, true)));
        }

        void operator()(llvm::Value *arg) { add(arg); }

    private:
        void add(llvm::Value *arg) { d_args.push_back(arg); }
